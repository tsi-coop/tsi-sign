...
# self-signed .pfx by design (see docs/architecture.md §6.2) - a real deployment swaps in the org's own
...
#
# A second, distinct keypair under alias "tsi_asp_signing" is this deployment's
# ASP request-signing key: the CCA eSign API requires every request an ASP
# sends to a CCA-licensed eSign Service Provider to be XML-DSig signed
# (CcaEsignAdapter). Self-signed here so the bundled TSI eSign Sandbox
# (sandbox/) accepts it; against a real CA, replace it with the certificate
# that CA issued to your ASP (see docs/architecture.md §6.3).
RUN mkdir -p /etc/tsi-sign && \
    ...
    keytool -genkeypair \
        -alias tsi_asp_signing \
        -keyalg RSA -keysize 2048 -validity 3650 \
        -dname "CN=TSI Sign ASP (dev), OU=TSI Coop, O=TSI Coop, L=Coimbatore, ST=TN, C=IN" \
        -keystore /etc/tsi-sign/keystore.p12 -storetype PKCS12 \
        -storepass changeit -keypass changeit && \
